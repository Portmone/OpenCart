...
class ControllerExtensionPaymentPortmone extends Controller {
    ...
    public function index() {
        ...
        if ($this->config->get('payment_portmone_entry_test_mode_flag') == 1) {
        ...
        $dt = date('Ymdhis');
        $signature = $this->config->get('payment_portmone_payee_id') . $dt . bin2hex($shop_order_number) . $bill_amount;
        ...
        $postData['payee'] = [
            'payeeId' =>  $this->config->get('payment_portmone_payee_id'),
        ...
        $postData['order'] = [
            ...
            'successUrl' => $this->url->link('extension/payment/portmone/callback', '', 'SSL'),
            'failureUrl' => $this->url->link('extension/payment/portmone/callback', '', 'SSL'),
            'preauthFlag' => ($this->config->get('payment_portmone_entry_preauth_flag') == 1)? 'Y' : 'N',
            'billCurrency' => $this->getBillCurrencyCode($order_info['currency_code']),
            'expTime' => $this->config->get('payment_portmone_exp_time'),
            'attribute1' => $attribute1,
            'attribute2' => ($this->config->get('payment_portmone_entry_client_phone_number_flag') == 1)? $order_info['telephone'] : '',
            'attribute3' => ($this->config->get('payment_portmone_entry_client_email_flag') == 1)? $order_info['email'] : '' ,
        ];
        ...
        $buttonPay = $this->language->get('button_pay');

        // alternative link payment page
        if ($this->config->get('payment_portmone_entry_alternative_link_payment_page_flag') == 1) {
            $str = 'payeeId=' . $postData['payee']['payeeId'] . '&'
                . 'bill_currency=' . $postData['order']['billCurrency'] . '&'
                . 'amount=' . $postData['order']['billAmount'] . '&'
                . 'shopOrderNumber=' . $postData['order']['shopOrderNumber'] . '&'
                . 'description=' .  $postData['order']['description'] . '&'
                . 'preauth_flag=' .  $postData['order']['preauthFlag'] . '&'
                . 'successUrl=' .  $postData['order']['successUrl'] . '&'
                . 'failureUrl=' . $postData['order']['failureUrl']
            ;
            ...
            $alternative_payment_page_link_data['button_pay'] = $buttonPay;
